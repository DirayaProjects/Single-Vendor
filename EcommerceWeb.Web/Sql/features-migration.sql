...
/* Example spin wheel prizes */
IF NOT EXISTS (SELECT 1 FROM dbo.SpinWheelPrizes)
BEGIN
    INSERT INTO dbo.SpinWheelPrizes (Label, DiscountPercent, Weight, Color, SortOrder) VALUES
        (N'5% OFF',   5.00,  30, N'#4CAF50', 1),
        (N'10% OFF', 10.00,  25, N'#2196F3', 2),
        (N'15% OFF', 15.00,  20, N'#FF9800', 3),
        (N'20% OFF', 20.00,  15, N'#9C27B0', 4),
        (N'$5 OFF',   NULL,   7, N'#F44336', 5),
        (N'Try Again', 0.00,  3, N'#9E9E9E', 6);

    UPDATE dbo.SpinWheelPrizes
        SET DiscountAmount = 5.00
        WHERE Label = N'$5 OFF';
END;
GO

/* Example promo ad */
IF NOT EXISTS (SELECT 1 FROM dbo.PromoAds)
BEGIN
    INSERT INTO dbo.PromoAds (Title, Subtitle, Description, ButtonText, SortOrder)
    VALUES
        (N'Summer Sale', N'Up to 30% off', N'Limited time offers on selected items.', N'Shop Deals', 1),
        (N'Free Delivery', N'Beirut only', N'Free delivery on orders over $50 in Beirut.', N'Learn More', 2),
        (N'New Arrivals', N'Fresh stock', N'Check out our latest products this week.', N'View All', 3);
END;
GO
...
/* ============================================================
   7) CART ITEM SELECTED ATTRIBUTES
   Store chosen attribute values per cart line; allow same product
   with different options as separate lines.
   ============================================================ */
...
PRINT 'CartItems: SelectedAttributes column added; unique (UserId, ProductId) removed.';
GO
