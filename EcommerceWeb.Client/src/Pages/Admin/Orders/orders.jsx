...
import { FaSearch, FaTrash, FaEdit, FaDownload, FaShoppingCart } from "react-icons/fa";
import { saveAs } from "file-saver";
import jsPDF from "jspdf";
import "jspdf-autotable";
import dayjs from "dayjs";
import { getOrders, updateOrder, deleteOrder as deleteOrderApi } from "../../../services/ordersApi";
...
const Orders = () => {
  ...
  const exportCSV = () => {
    ...
    filteredOrders.forEach((o) => {
      csv += `${o.id},${o.customer},${formatTotal(o.total)},${o.status},${o.date},${o.description || ""}\n`;
    });
  ...
  const exportWord = () => {
    ...
    filteredOrders.forEach((o) => {
      content += `<tr><td>${o.id}</td><td>${o.customer}</td><td>${formatTotal(o.total)}</td><td>${o.status}</td><td>${o.date}</td><td>${o.description || ""}</td></tr>`;
    });
    content += "</table>";
  ...
  const printOrders = () => {
    ...
    filteredOrders.forEach((o) => {
      html += `<tr><td>${o.id}</td><td>${o.customer}</td><td>${formatTotal(o.total)}</td><td>${o.status}</td><td>${o.date}</td><td>${o.description || ""}</td></tr>`;
    });
    html += "</table>";
  ...
  const saveEdit = async () => {
    ...
    try {
      const payload = {
        ...
        total: parseTotal(formData.total),
        status: formData.status,
        date: dayjs(formData.date).format("YYYY-MM-DD"),
        description: formData.description,
      };
      const saved = await updateOrder(currentOrder.id, payload);
