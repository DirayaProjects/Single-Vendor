...
import { getAttributes } from "../../../services/attributesApi";
          ...
          <div className="product-attributes">
            {Object.entries(product.attributes).map(([key, values]) => {
              ...
              return list.filter(Boolean).map((value) => (
                <span key={`${key}-${value}`} className="chip attr-chip">{key}: {value}</span>
              ));
            })}
          </div>
        )}
...
export default function ProductsPage() {
  ...
  const handleSaveProduct = async (prod) => {
    try {
      const payload = toPayload(prod);
      if (prod.id) {
        const saved = await updateProduct(prod.id, payload);
      ...
      <main className="products-page" style={{ marginLeft: sidebarOpen ? 220 : 60 }}>
        ...
        <div className="pagination">
          ...
          {Array.from({ length: totalPages }).map((_, i) => {
            ...
            if (p === page - 3 || p === page + 3) return <span key={p} className="dots">…</span>;
            return null;
          })}
